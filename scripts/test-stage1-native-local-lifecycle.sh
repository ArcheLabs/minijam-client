...
spec_checksum="$(sha256sum "${RUNTIME}/stage1-direct-e2e.json" | awk '{print $1}')"
...
test -f "${RUNTIME}/worker-recovery.json" || true
...
test "${spec_checksum}" = "$(sha256sum "${RUNTIME}/stage1-direct-e2e.json" | awk '{print $1}')"
