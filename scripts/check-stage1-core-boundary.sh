...
forbidden='clang|llvm|libclang|apt\.llvm\.org|llvm\.sh|deploy/compiler|compile-service|test-compiler-image|test-counter-services|cargo test --workspace|cargo build --release -p minijam-node|cargo build --release -p minijam-worker|export-stage0-chain-specs|stage0-artifacts'
