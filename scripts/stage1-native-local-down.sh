...
# Stop the Worker before the services it depends on, then stop Formal RPC and
# the node. The chain database, bundles, logs, and recovery database
# remain in place unless the operator explicitly requests a purge.
stop_process "${RUNTIME}/worker.pid"
printf 'MINIJAM_LOCAL_WORKER=STOPPED\n'
