...
for stage in rust-runtime node playground playground-api-release worker compiler-binary; do
  if extract_stage "${stage}" | grep -Eni "${toolchain}"; then
    echo "production target ${stage} contains compiler-toolchain material" >&2
    exit 1
  fi
done

extract_stage compiler | grep -Eq 'silkeh/clang:20-bullseye@sha256:302c1c6d5cfd72ee154696a11e097bcc3a20e7060dac9add9a75dfbe8319947b'
if grep -En 'apt\.llvm\.org|llvm\.sh' "${ROOT}/deploy/compiler/Dockerfile" "${DOCKERFILE}"; then
...
stage1_runtime_toolchain='cargo|rustc|clang|clang\+\+|llvm-config|libclang|protobuf-compiler|apt\.llvm\.org|llvm\.sh'

for stage in node worker formal-rpc; do
  if awk -v stage="${stage}" '
      /^FROM / { capture = ($0 ~ " AS " stage "([[:space:]]|$)") }
      capture { print }
    ' "${STAGE1_DOCKERFILE}" | grep -Eni "${stage1_runtime_toolchain}"; then
    echo "Stage-1 production target ${stage} contains build-toolchain material" >&2
    exit 1
  fi
done
...
if grep -Eni 'playground|compiler-api|jam.?computer|minicells|faucet' "${STAGE1_DOCKERFILE}"; then
