...
| 路径 | 职责 |
| --- | --- |
| `crates/minijam-protocol` | 协议常量、内容引用、规范报告和状态变更 |
| `crates/minijam-jamcore-api` | 版本化 JamCore 接口和执行类型 |
| `crates/minijam-worker` | Worker daemon 和 bundle 获取 |
| `crates/minijam-formal-rpc` | 应用无关的 transaction queue 与 bundle gateway |
...
Formal RPC 负责持久 transaction queue 和 bundle store；单一 Worker 持有配置的
WorkerAccount，并直接向 node 提交规范报告。Node RPC 和 compiler 端点应保持在部署的
私有边界内。
