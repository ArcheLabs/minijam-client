...
| `crates/minijam-launcher` | 聚合的 `minijam --dev` 本地网络 supervisor |
...
生产部署单元是精确的镜像 digest 和与之匹配的 `testnet` chain spec。镜像仓库为：
...
testnet 单机部署使用 `deploy/stage1/compose.compact.yml`，跨主机私有网络使用
`deploy/stage1/compose.split.yml`。使用精确的 node 镜像运行
`scripts/export-testnet-chain-specs-image.sh` 生成 `testnet.json` 和
`testnet-raw.json`；生成文件不提交到仓库。
