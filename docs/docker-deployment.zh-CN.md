...
精确 node 镜像生成的确定性 `testnet` chain specification。本地开发使用聚合
镜像 `minijam --dev`，不需要 Compose。

## 组件

testnet 的 compact 和 split 配置包含相同的三个角色：
...
本地聚合镜像还包含 launcher、node、worker 和 Formal RPC，并且只启动一个
Worker：
...
从 Stage-1 发布产物获取三个组件镜像 digest，再从精确 node 镜像生成匹配的
testnet chain spec：
...
设置镜像和 secret 值，然后验证并启动。节点直接使用内置的 `testnet` chain
spec，不挂载生成文件。Compose 会把这些由环境
...
在参与部署的主机上创建共享的外部 `chain` 网络。使用相同的镜像 digest 和
secret 约定运行 `deploy/stage1/compose.split.yml`，并将
