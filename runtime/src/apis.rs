...
impl_runtime_apis! {
    ...
    impl minijam_rpc_runtime_api::MiniJamRuntimeApi<Block> for Runtime {
        fn get_package_status(package_hash: minijam_protocol::Hash) -> Option<minijam_protocol::PackageStatus> {
            pallet_minijam::Pallet::<Runtime>::get_package_status(package_hash)
        }
        fn get_package_failure(package_hash: minijam_protocol::Hash) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_package_failure(package_hash).map(|value| value.encode())
        }
        fn get_execution_receipt_by_package_hash(package_hash: minijam_protocol::Hash) -> Option<minijam_protocol::Hash> {
            pallet_minijam::Pallet::<Runtime>::get_execution_receipt_by_package_hash(package_hash)
        }
        ...
        fn get_allocation(allocation_id: u64) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_allocation(allocation_id).map(|value| value.encode())
        }
        ...
        fn has_pending_preimage(requester: u32, blob_hash: minijam_protocol::Hash, blob_len: u32) -> bool {
            pallet_minijam::Pallet::<Runtime>::has_pending_preimage(requester, blob_hash, blob_len)
        }
        ...
        fn get_system_op(request_id: minijam_protocol::Hash) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_system_op(request_id).map(|value| value.encode())
        }
        fn get_system_receipt(request_id: minijam_protocol::Hash) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_system_receipt(request_id).map(|value| value.encode())
        }
        fn get_system_op_nonce(sender: minijam_protocol::Hash) -> u64 {
            pallet_minijam::Pallet::<Runtime>::get_system_op_nonce(sender)
        }
        fn get_system_service_info() -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_system_service_info().map(|value| value.encode())
        }
        fn get_service_info(service_id: u32) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_service_info(service_id).map(|value| value.encode())
        }
        fn get_service_storage(service_id: u32, key: Vec<u8>) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_service_storage(service_id, key).map(|value| value.encode())
        }
        fn get_service_preimage(service_id: u32, code_hash: minijam_protocol::Hash) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_service_preimage(service_id, code_hash).map(|value| value.encode())
        }
        fn get_protocol_state(key: [u8; 31]) -> Option<Vec<u8>> {
            pallet_minijam::Pallet::<Runtime>::get_protocol_state(key).map(|value| value.encode())
        }
    }
    ...
    impl sp_block_builder::BlockBuilder<Block> for Runtime {
        ...
        fn check_inherents(block: <Block as BlockT>::LazyBlock, data: sp_inherents::InherentData) -> sp_inherents::CheckInherentsResult { data.check_extrinsics(&block) }
    }
    ...
    impl sp_session::SessionKeys<Block> for Runtime {
        fn generate_session_keys(owner: Vec<u8>, seed: Option<Vec<u8>>) -> OpaqueGeneratedSessionKeys { SessionKeys::generate(&owner, seed).into() }
        fn decode_session_keys(encoded: Vec<u8>) -> Option<Vec<(Vec<u8>, KeyTypeId)>> { SessionKeys::decode_into_raw_public_keys(&encoded) }
    }
    ...
    impl sp_consensus_grandpa::GrandpaApi<Block> for Runtime {
        ...
        fn submit_report_equivocation_unsigned_extrinsic(
            _equivocation_proof: sp_consensus_grandpa::EquivocationProof<<Block as BlockT>::Hash, NumberFor<Block>>,
            _key_owner_proof: sp_consensus_grandpa::OpaqueKeyOwnershipProof,
        ) -> Option<()> { None }
        fn generate_key_ownership_proof(_set_id: sp_consensus_grandpa::SetId, _authority_id: GrandpaId) -> Option<sp_consensus_grandpa::OpaqueKeyOwnershipProof> { None }
    }
    ...
    impl pallet_transaction_payment_rpc_runtime_api::TransactionPaymentApi<Block, Balance> for Runtime {
        fn query_info(uxt: <Block as BlockT>::Extrinsic, len: u32) -> pallet_transaction_payment_rpc_runtime_api::RuntimeDispatchInfo<Balance> { TransactionPayment::query_info(uxt, len) }
    ...
    impl sp_genesis_builder::GenesisBuilder<Block> for Runtime {
        ...
        fn get_preset(id: &Option<sp_genesis_builder::PresetId>) -> Option<Vec<u8>> { get_preset::<RuntimeGenesisConfig>(id, crate::genesis_config_presets::get_preset) }
        fn preset_names() -> Vec<sp_genesis_builder::PresetId> { crate::genesis_config_presets::preset_names() }
    }
}
